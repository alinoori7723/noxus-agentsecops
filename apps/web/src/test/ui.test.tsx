...
import { ProviderSettings, baseUrlError, BASE_URL_SCHEME_ERROR, type ProviderTestState, } from "../components/ProviderSettings";
...
import { conditionalPassSummary, evidenceWithProprietaryRisk, redBlueWithRealRail, deterministicTrace, agentTrace, providerTestSuccess, providerTestFailure, } from "./fixtures";
describe("Overview", () => {
    it("renders the product positioning and scope honesty", () => {
        ...
        expect(screen.getByText(/Pre-production AI security readiness testing/i)).toBeInTheDocument();
...
describe("Sidebar", () => {
    ...
    it("invokes onSelect when a section is clicked", () => {
        ...
        render(<Sidebar active="overview" onSelect={(id) => (picked = id)} hasResult={false}/>);
...
function AssessmentHarness() {
    ...
    return (<AssessmentPanel mode={mode} onModeChange={setMode} provider={{ provider_type: "local_openai_compatible", api_key: "" }} onGoToProvider={() => { }} onRun={() => { }} running={false} error={null} providerTestOk={false} providerTestStale={false} providerConfigError={null}/>);
}
...
function ProviderHarness({ type = "gemini_native" as ProviderConfig["provider_type"], testState = {
    status: "idle",
    response: null,
    error: null,
    stale: false,
} as ProviderTestState, onTest = (_roles: AgentRole[]) => { }, }: {
    ...
    return (<ProviderSettings provider={provider} onChange={setProvider} testState={testState} onTest={onTest}/>);
}
describe("ProviderSettings", () => {
    ...
    it("renders Gemini presets and a password API key field", () => {
        ...
        expect(screen.getByText(/not stored in browser storage, reports, or audit export/i)).toBeInTheDocument();
    });
    ...
    it("renders a successful provider diagnostic result", () => {
        render(<ProviderHarness testState={{ status: "done", response: providerTestSuccess, error: null, stale: false }}/>);
    ...
    it("renders a failed provider diagnostic result with a sanitized message", () => {
        render(<ProviderHarness type="openai_compatible" testState={{ status: "done", response: providerTestFailure, error: null, stale: false }}/>);
...
describe("RoleObservability", () => {
    it("shows LLM roles as not used / deterministic baseline in deterministic mode", () => {
        render(<RoleObservability trace={deterministicTrace} evidence={evidenceWithProprietaryRisk}/>);
...
describe("ReadinessSummary", () => {
    it("shows CONDITIONAL_PASS honestly (amber, not promoted to PASS)", () => {
        ...
        expect(screen.getByText(/CONDITIONAL_PASS — improved, but human review still required/i)).toBeInTheDocument();
        expect(screen.getByText(/rather than returning a fake PASS/i)).toBeInTheDocument();
    });
});
describe("OpenRisks", () => {
    it("renders proprietary-context exposure as an unresolved open risk", () => {
        render(<OpenRisks model={evidenceWithProprietaryRisk}/>);
        expect(screen.getByText(/Proprietary-context exposure is not auto-patched/i)).toBeInTheDocument();
...
describe("RedBlueDashboard", () => {
    it("renders detection labels and real safety-rail telemetry (no placeholder)", () => {
        ...
        expect(screen.queryByText(/<critical safety rail clause>/i)).not.toBeInTheDocument();
    });
});
describe("baseUrlError", () => {
    it("rejects a base URL without an http(s) scheme", () => {
        expect(baseUrlError({ provider_type: "local_openai_compatible", base_url: "localhost:4000/v1" })).toBe(BASE_URL_SCHEME_ERROR);
        expect(baseUrlError({ provider_type: "openai_compatible", base_url: "localhost:4000/v1" })).toBe(BASE_URL_SCHEME_ERROR);
    });
    it("accepts http/https, blank-local, and gemini (no base URL)", () => {
        expect(baseUrlError({ provider_type: "local_openai_compatible", base_url: "http://localhost:4000/v1" })).toBeNull();
...
describe("ProviderSettings base URL validation", () => {
    it("shows an inline error and disables Test (no call) when the scheme is missing", () => {
        const onTest = vi.fn();
        render(<ProviderSettings provider={{
                provider_type: "openai_compatible",
                base_url: "localhost:4000/v1",
                api_key: "k",
            }} onChange={() => { }} testState={{ status: "idle", response: null, error: null, stale: false }} onTest={onTest}/>);
    ...
    it("shows local + Docker helper examples when the base URL is valid", () => {
        render(<ProviderSettings provider={{ provider_type: "local_openai_compatible", base_url: "http://localhost:4000/v1", api_key: "k" }} onChange={() => { }} testState={{ status: "idle", response: null, error: null, stale: false }} onTest={() => { }}/>);
...
describe("PolicyError", () => {
    it("renders a friendly validation message and the unsupported keys", () => {
        ...
        expect(screen.getByText(/does not match the supported Noxus policy schema/i)).toBeInTheDocument();
...
describe("PartialRunBanner (HUMAN_REVIEW_REQUIRED partial)", () => {
    it("shows the failed agent stage and that the deterministic baseline is preserved", () => {
        ...
        expect(screen.getByText(/deterministic baseline.*completed/i)).toBeInTheDocument();
...
describe("RoleObservability partial failure", () => {
    it("marks the failed agent role as failed", () => {
        render(<RoleObservability trace={agentTracePartialFailure} evidence={evidenceWithProprietaryRisk}/>);
        expect(screen.getByText("Red Team Agent")).toBeInTheDocument();
...
import { agentTraceRedFallbackContinued, redTeamFailureContinued, redBlueNoPatch, baselineTimeline, } from "./fixtures";
describe("RedTeamFallbackNotice (Red failed, continued on baseline)", () => {
    it("shows the fallback message and that baseline evidence was used", () => {
        ...
        expect(screen.getByText(/Red Team Agent failed schema validation\. Noxus continued using deterministic baseline evidence/i)).toBeInTheDocument();
...
describe("RoleObservability after Red failure + fallback", () => {
    it("shows Red failed, judge skipped, tuning used, and the fallback note", () => {
        render(<RoleObservability trace={agentTraceRedFallbackContinued} evidence={evidenceWithProprietaryRisk}/>);
        expect(screen.getByText("Red Team Agent")).toBeInTheDocument();
        expect(screen.getAllByText(/failed/i).length).toBeGreaterThan(0);
        expect(screen.getAllByText(/skipped/i).length).toBeGreaterThan(0);
        expect(screen.getAllByText(/^used$/i).length).toBeGreaterThan(0);
        expect(screen.getAllByText(/continued using\s+deterministic baseline evidence/i)
...
describe("AuditTimeline (baseline preserved)", () => {
    it("is not blank when the deterministic baseline exists", () => {
        ...
        expect(screen.getAllByText(/evidence items/i).length).toBe(baselineTimeline.length);
    });
});
describe("RedBlueDashboard safety-rail honesty", () => {
    it("shows no fake safety rail preview when no patch was applied", () => {
        ...
        expect(screen.getByText(/No safety rail preview available from report data/i)).toBeInTheDocument();
...
import { agentTraceJudgeDegraded, } from "./fixtures";
describe("Evidence basis (degraded/fallback clarity)", () => {
    it("ui_shows_evidence_basis_for_fallback_run", () => {
        render(<RoleObservability trace={agentTraceRedFallbackContinued} evidence={evidenceWithProprietaryRisk}/>);
        expect(screen.getByText(/Evidence basis: deterministic baseline fallback/i)).toBeInTheDocument();
    });
    it("normal_agent_run_does_not_show_fallback_basis", () => {
        render(<RoleObservability trace={agentTrace} evidence={evidenceWithProprietaryRisk}/>);
        expect(screen.getByText(/Evidence basis: red-team augmented/i)).toBeInTheDocument();
        expect(screen.queryByText(/deterministic baseline fallback/i)).not.toBeInTheDocument();
        expect(screen.queryByText(/failed schema validation/i)).not.toBeInTheDocument();
    });
    it("shows Semantic Judge failed + degraded note when the judge degraded", () => {
        render(<RoleObservability trace={agentTraceJudgeDegraded} evidence={evidenceWithProprietaryRisk}/>);
        expect(screen.getAllByText(/failed/i).length).toBeGreaterThan(0);
        expect(screen.getByText(/Semantic Judge failed schema validation/i)).toBeInTheDocument();
        expect(screen.getAllByText(/^used$/i).length).toBeGreaterThan(0);
        expect(screen.getByText(/Evidence basis: red-team augmented/i)).toBeInTheDocument();
    });
});
...
describe("RedBlueDashboard remediation honesty (resolved vs unresolved)", () => {
    it("patch_operations_show_source_finding_ids", () => {
        render(<RedBlueDashboard model={redBlueWithRealRail}/>);
        expect(screen.getByText(/probe_indirect_prompt_injection:indirect_prompt_injection_simulated/)).toBeInTheDocument();
    });
    ...
    it("ui_explains_zero_after_score_when_patches_applied", () => {
        ...
        expect(screen.getByText(/Patches were applied, but blocking findings remained in retest/i)).toBeInTheDocument();
...
describe("ReadinessSummary readiness-gate vs remediation-progress (Fix 1)", () => {
    ...
    it("ui_explains_zero_after_score_when_resolved_findings_exist", () => {
        const zeroGate = {
            ...
            after_score: 0,
            after_score_explanation: "Readiness remains blocked because high-risk findings remain. Remediation progress is shown separately.",
        };
        render(<ReadinessSummary model={zeroGate}/>);
        expect(screen.getByText(/Readiness remains blocked because high-risk findings remain/i)).toBeInTheDocument();
...
describe("RedBlueDashboard primary lineage + derivation (Fix 2 & 3)", () => {
    ...
    it("ui_renders_human_review_derivation", () => {
        render(<RedBlueDashboard model={redBlueWithRealRail}/>);
        expect(screen.getByText(/Human review requirements \(derived from unresolved findings\)/i)).toBeInTheDocument();
        expect(screen.getByText(/derived from retest/i)).toBeInTheDocument();
        expect(screen.getByText(/Derived from 1 unresolved retest finding/i)).toBeInTheDocument();
    });
...
describe("ReportSummary judge-safe summary (Fix 4)", () => {
    ...
    it("report_summary_explains_no_pass_with_demo_copy", () => {
        render(<ReportSummary model={conditionalReportSummary}/>);
        expect(screen.getByText(/Noxus did not mark this target safe/i)).toBeInTheDocument();
        expect(screen.getByText(/Final state is not PASS because unsupported or unresolved/i)).toBeInTheDocument();
    });
...
import { tuningTimeoutFailure, recoveredTuningFallback, providerTestTimeout, } from "./fixtures";
describe("TimeoutNotice role-specific LLM timeout (Fix 1 & 4)", () => {
    it("ui_renders_role_specific_timeout_message", () => {
        ...
        expect(screen.getByText(/timed out during Policy Tuning Agent using gemini-3.1-pro-preview/i)).toBeInTheDocument();
    ...
    it("renders the tuning fallback note (fallback does not hide the timeout)", () => {
        render(<TimeoutNotice failure={null} fallback={recoveredTuningFallback}/>);
        expect(screen.getByText(/timed out on/i)).toBeInTheDocument();
        expect(screen.getByText("gemini-3.1-pro-preview")).toBeInTheDocument();
...
describe("Provider diagnostics timeout vs schema failure (Fix 6)", () => {
    it("provider_test_distinguishes_timeout_from_schema_failure_in_ui", () => {
        render(<ProviderHarness type="gemini_native" testState={{ status: "done", response: providerTestTimeout, error: null, stale: false }}/>);
...
describe("ReadinessSummary readiness-score, not risk-score (KPI Fix 1)", () => {
    ...
    it("report_summary_explains_readiness_score_direction", () => {
        ...
        expect(screen.getByText(/Risk is represented separately/i)).toBeInTheDocument();
    });
});
describe("RedBlueDashboard related findings + probe mapping (KPI Fix 3 & 4)", () => {
    it("ui_uses_related_findings_not_secondary_sources_for_non_primary_lineage", () => {
        ...
        expect(screen.getByText(/Related findings from same category:/i)).toBeInTheDocument();
    ...
    it("ui_shows_probe_finding_mapping_matrix", () => {
        ...
        expect(screen.getByText(/one probe may emit multiple findings/i)).toBeInTheDocument();
        expect(screen.getByText(/Baseline: 5 failed probes -> 6 finding instances/i)).toBeInTheDocument();
        expect(screen.getByText(/Retest: 1 failed probes -> 1 unresolved finding instances/i)).toBeInTheDocument();
    });
...
describe("RedBlueDashboard BLOCKED gate + positive delta (KPI Fix 5)", () => {
    it("ui_does_not_present_positive_delta_as_pass", () => {
        render(<RedBlueDashboard model={redBlueZeroAfterScore}/>);
        expect(screen.getByText(/Readiness improved, but deployment remains blocked because unresolved high-risk findings remain/i)).toBeInTheDocument();
