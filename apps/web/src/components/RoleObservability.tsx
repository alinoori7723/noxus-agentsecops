...
import type { AgentTrace, AgentTraceStage, ChipColor, EvidenceModel, StageStatus, } from "../types/noxus";
...
function RoleCard({ stage, judgeConfidence, }: {
    ...
    return (<div className={"card p-4 " +
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <StatusChip label={STATUS_LABEL[stage.status]} color={STATUS_COLOR[stage.status]}/>
          </div>
      ...
      <dl className="mt-3 space-y-1 text-[12px]">
        {!engine && (<div className="flex justify-between gap-2">
        ...
        {stage.stage === "semantic_judge" && judgeConfidence && (<div className="flex justify-between gap-2">
...
export function RoleObservability({ trace, evidence, }: {
    trace: AgentTrace;
    evidence: EvidenceModel;
}) {
    const judgeConfidence = evidence.findings.find((f) => f.detection_mode === "semantic_llm")?.confidence ??
        null;
    ...
    return (<div className="card p-5">
      <SectionHeader kicker="LLM role observability" title="Which model did what" copy={trace.execution_mode === "agent_assisted"
            ? `Agent-Assisted run via ${trace.provider_type}. Each LLM role and the deterministic engine are shown separately.`
            : "Deterministic run — no live LLM calls. The deterministic baseline and patch engine did the work; LLM roles were not used."} right={<StatusChip label={trace.continued_after_red_failure
                ? "agent-assisted (degraded)"
                : trace.execution_mode === "agent_assisted"
                    ? "agent-assisted"
                    : "deterministic"} color={trace.continued_after_red_failure
                ? "amber"
                : trace.execution_mode === "agent_assisted"
                    ? "blue"
                    : "neutral"}/>}/>
      {trace.continued_after_red_failure && (<p className="mb-3 rounded-lg border border-amber-200 bg-amber-50/70 px-3 py-2 text-[12px] font-medium text-amber-800" role="status">
      ...
      {trace.execution_mode === "agent_assisted" && trace.evidence_basis && (<p className="mb-3 text-[12px] font-semibold text-slate-600">
          Evidence basis: {EVIDENCE_BASIS_LABEL[trace.evidence_basis]}
        </p>)}
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {stages.map((s) => (<RoleCard key={s.stage} stage={s} judgeConfidence={judgeConfidence}/>))}
      </div>
    </div>);
}
