...
export function ReportSummary({ model }: {
    ...
    return (<div className="card overflow-hidden">
      ...
      {model.summary_copy && (<p className="border-b border-slate-100 px-5 py-3 text-sm font-medium leading-relaxed text-slate-700">
          {model.summary_copy}
        </p>)}
      ...
      <div className="grid gap-4 p-5 md:grid-cols-2">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {improved.primary_finding_types_resolved.length > 0 ? (improved.primary_finding_types_resolved.map((t) => (<StatusChip key={t} label={t} color="green" mono/>))) : (<p className="text-[11px] text-slate-400">
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {blocked.unresolved_finding_types.length > 0 ? (blocked.unresolved_finding_types.map((t) => (<StatusChip key={t} label={t} color="amber" mono/>))) : (<p className="text-[11px] text-slate-400">None.</p>)}
          </div>
          <p className="mt-2 text-[12px] text-slate-500">Human review categories</p>
          <div className="mt-1 flex flex-wrap gap-1.5">
            {blocked.human_review_categories.length > 0 ? (blocked.human_review_categories.map((c) => (<StatusChip key={c} label={c} color="amber"/>))) : (<p className="text-[11px] text-slate-400">None.</p>)}
          </div>
        </div>
      </div>

      {model.why_not_pass && (<div className="flex items-start gap-2 border-t border-rose-100 bg-rose-50/60 px-5 py-3 text-[12.5px] font-medium text-rose-800" role="note">
