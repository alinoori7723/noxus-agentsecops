import { FileCheck2, Cog, RefreshCcw, UserCheck, ScanSearch, Container, FileJson, LayoutPanelTop, FlaskConical, type LucideIcon, } from "lucide-react";
import type { SafeguardItem } from "../types/noxus";
const STATIC_SAFEGUARDS: {
    title: string;
    detail: string;
    icon: LucideIcon;
}[] = [
    ...
    },
    {
        ...
        detail: "The packaged container runs as a non-root user on Python 3.11 on Alpine Linux.",
        icon: Container,
    },
    {
    ...
    {
        title: "Continuous quality and security gates",
        detail: "Regression tests, coverage thresholds, type checks, dependency audits, and container checks run in CI.",
        icon: FlaskConical,
    },
];
export function EngineeringSafeguards({ items }: {
    items: SafeguardItem[];
}) {
    ...
    return (<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {cards.map(({ title, detail, icon: Icon }) => (<div key={title} className="card p-4">
