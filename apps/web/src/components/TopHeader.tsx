...
import { ShieldCheck, Layers, Container } from "lucide-react";
...
function Chip({ icon, children }: {
    ...
    return (<span className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">
...
export function TopHeader({ title, subtitle, proof, online }: TopHeaderProps) {
    ...
    return (<header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 px-6 py-3.5 lg:px-8">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Chip icon={<ShieldCheck size={13}/>}>Fail-closed evaluation</Chip>
          <Chip icon={<Layers size={13}/>}>At most {maxIter} tuning rounds</Chip>
