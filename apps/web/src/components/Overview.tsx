import { Swords, Wrench, FileText, ArrowRight, ShieldAlert, Play, SlidersHorizontal, } from "lucide-react";
    ...
    return (<div className="space-y-6">
      <section className="card p-6 lg:p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <button type="button" onClick={onRunDemo} disabled={running} className="btn-ghost">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {VALUE_CARDS.map((c) => {
            const Icon = c.icon;
            return (<div key={c.title} className="card p-5">
      ...
      <section className="card p-6">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {FLOW.map((step, i) => (<div key={step} className="flex items-center gap-2">
              <span className="rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm font-bold text-slate-800">
                {step}
              </span>
              {i < FLOW.length - 1 && (<ArrowRight size={16} className="text-slate-300"/>)}
            </div>))}
        </div>
      </section>
    </div>);
