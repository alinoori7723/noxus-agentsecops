...
export function Sidebar({ active, onSelect, hasResult }: SidebarProps) {
    return (<aside className="flex w-60 shrink-0 flex-col border-r border-slate-200 bg-white">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
        {NAV_ITEMS.map((item) => {
            ...
            return (<button key={item.id} type="button" onClick={() => onSelect(item.id)} aria-current={isActive ? "page" : undefined} className={clsx("nav-item", isActive && "nav-item-active")}>
              <Icon size={17} className={isActive ? "text-brand-600" : "text-slate-400"}/>
              <span className="flex-1 text-left">{item.label}</span>
              {showDot && (<span className="h-1.5 w-1.5 rounded-full bg-emerald-500" title="Results ready"/>)}
            </button>);
        })}
      </nav>
