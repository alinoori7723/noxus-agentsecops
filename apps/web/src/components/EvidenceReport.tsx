...
function FindingCard({ finding, openRisk }: {
    finding: FindingRow;
    openRisk: boolean;
}) {
    return (<div className="card p-4">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        ...
        {finding.confidence && (<StatusChip label={`confidence: ${finding.confidence}`} color="blue"/>)}
...
export function EvidenceReport({ model }: {
    model: EvidenceModel;
}) {
    ...
    return (<div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-1 rounded-lg border border-slate-200 bg-white p-1">
          {(["after", "before"] as const).map((t) => (<button key={t} type="button" onClick={() => setTab(t)} className={clsx("rounded-md px-3 py-1.5 text-xs font-semibold transition", tab === t
      ...
      {findings.length > 0 ? (<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {findings.map((f, i) => (<FindingCard key={i} finding={f} openRisk={tab === "after"}/>))}
        </div>) : (<div className="card p-6 text-sm text-slate-500">
