...
export function PolicyError({ detail, onResetPolicy }: PolicyErrorProps) {
    ...
    return (<div className="card border-rose-200 bg-rose-50/50 p-5" role="alert">
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {unsupported.length > 0 && (<div className="mt-3">
              <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
                Unsupported keys
              </div>
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {unsupported.map((k) => (<StatusChip key={k} label={k} color="red" mono/>))}
              </div>
            </div>)}

          {allowed.length > 0 && (<div className="mt-3">
              <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
                Supported top-level keys
              </div>
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {allowed.map((k) => (<StatusChip key={k} label={k} color="neutral" mono/>))}
              </div>
            </div>)}

          {detail.example_yaml && (<div className="mt-3">
              <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
          ...
          <button type="button" onClick={onResetPolicy} className="btn-ghost mt-4 text-rose-700 hover:border-rose-300">
