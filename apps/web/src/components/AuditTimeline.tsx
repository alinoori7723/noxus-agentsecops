...
export function AuditTimeline({ steps }: {
    steps: TimelineStep[];
}) {
    return (<div className="card p-5">
      <SectionHeader kicker="Audit timeline" title="Six-step readiness flow" copy="Each step is built only from real report fields — baseline, findings, structured patch proposal, deterministic application, retest, and final readiness."/>
      <ol className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {steps.map((s) => (<li key={s.step} className="flex flex-col rounded-lg border border-slate-200 bg-slate-50/60 p-3.5">
