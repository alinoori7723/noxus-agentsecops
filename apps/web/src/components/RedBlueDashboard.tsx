...
function ProbeCard({ probe }: {
    ...
    return (<div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {probe.evidence.length > 0 ? (<div className="mt-2 space-y-1.5">
          {probe.evidence.map((ev, i) => (<pre key={i} className="code-block scroll-thin max-h-24">
              {ev}
            </pre>))}
        </div>) : (<p className="mt-1.5 text-[11px] text-slate-400">
          No findings emitted for this probe.
        </p>)}
    </div>);
}
const PATCH_STATUS_META: Record<string, {
    label: string;
    color: ChipColor;
}> = {
...
function PatchCard({ patch }: {
    ...
    return (<div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {patch.related_finding_groups.same_category_related.length > 0 && (<p className="mt-0.5 text-[11px] text-slate-400">
          Related findings from same category:{" "}
          <span className="font-mono text-slate-500">
            {patch.related_finding_groups.same_category_related.join(", ")}
          </span>
        </p>)}
      {patch.related_finding_groups.leakage_from_same_probe.length > 0 && (<p className="mt-0.5 text-[11px] text-slate-400">
          Leakage from same probe:{" "}
          <span className="font-mono text-slate-500">
            {patch.related_finding_groups.leakage_from_same_probe.join(", ")}
          </span>
        </p>)}
      {patch.related_finding_groups.generic_policy_related.length > 0 && (<p className="mt-0.5 text-[11px] text-slate-400">
...
export function RedBlueDashboard({ model }: {
    model: RedBlueModel;
}) {
      ...
      <div className="grid lg:grid-cols-2">
        ...
        <div className="border-b border-slate-200 lg:border-b-0 lg:border-r">
          <div className="border-b border-rose-100 bg-rose-50/60 px-5 py-3.5">
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              <StatusChip label={`baseline failures: ${red.before_summary.failed_probes}`} color="red"/>
              <StatusChip label={`retest failures: ${red.after_summary.failed_probes}`} color="amber"/>
              <StatusChip label={`retest findings: ${red.after_summary.findings}`} color="neutral"/>
            </div>
          </div>
          <div className="space-y-3 p-4">
            <div className="kicker">Before-state failures</div>
            {failing.map((p) => (<ProbeCard key={`b-${p.probe_id}`} probe={p}/>))}
            <div className="kicker pt-1">Retest probes</div>
            {red.retest_probes.map((p) => (<ProbeCard key={`r-${p.probe_id}`} probe={p}/>))}
          </div>
        </div>
        ...
        <div>
          <div className="border-b border-sky-100 bg-sky-50/60 px-5 py-3.5">
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              <StatusChip label={`patch operations: ${blue.patches.length}`} color="blue"/>
              <StatusChip label="deterministic engine" color="green"/>
              <StatusChip label={`human review: ${blue.human_review_requirements.length}`} color="amber"/>
            </div>
          </div>
          <div className="space-y-3 p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              <StatusChip label={`readiness gate: ${rem.readiness_gate}`} color={rem.readiness_gate === "PASS" ? "green" : "amber"}/>
              <StatusChip label={`readiness gate score: ${rem.readiness_gate_score}/100`} color={rem.readiness_gate_score > 0 ? "green" : "amber"}/>
              <StatusChip label={`remediation progress: ${rem.remediation_progress.label}`} color={rem.remediation_progress.resolved > 0 ? "green" : "neutral"}/>
            </div>
            {rem.after_score_explanation && (<p className="text-[12px] font-medium text-amber-700">
                {rem.after_score_explanation}
              </p>)}
            {rem.gate_blocked_explanation && (<div className="rounded-lg border border-amber-300 bg-amber-50/70 p-3 text-[12px] font-medium text-amber-800" role="note">
                <p>{rem.gate_blocked_explanation}</p>
                <p className="mt-0.5 text-amber-700">Gate remains blocked.</p>
                {rem.gate_blocking_reason && (<p className="mt-0.5 font-mono text-[11px] text-amber-700">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50/60 p-3 text-[12px] text-slate-600">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                <StatusChip label={`Baseline: ${rem.probe_finding_mapping.baseline_label}`} color="red"/>
                <StatusChip label={`Retest: ${rem.probe_finding_mapping.retest_label}`} color="amber"/>
                <StatusChip label={rem.probe_finding_mapping.resolved_label} color="green"/>
                <StatusChip label={`Unresolved types: ${rem.probe_finding_mapping.unresolved_finding_types.join(", ") || "none"}`} color="neutral"/>
              </div>
              <p className="mt-2 text-[11px] text-slate-500">
            ...
            <div className="flex flex-wrap gap-1.5">
              <StatusChip label={`patch operations applied: ${rem.patch_application_count}`} color="blue"/>
              <StatusChip label={`resolved findings: ${rem.resolved_finding_count}`} color="green"/>
              <StatusChip label={`unresolved findings: ${rem.unresolved_finding_count}`} color={rem.unresolved_finding_count > 0 ? "amber" : "green"}/>
              {rem.rejected_proposal_count > 0 && (<StatusChip label={`rejected proposals: ${rem.rejected_proposal_count}`} color="red"/>)}
            </div>
            {blue.blocking_explanation && (<div className="flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50/70 p-3 text-[12px] font-medium text-amber-800" role="alert">
            ...
            {blue.patches.map((p, i) => (<PatchCard key={i} patch={p}/>))}

            {blue.rejected_proposals.length > 0 && (<>
                <div className="kicker pt-1">
                  Rejected / unlinked proposals (not applied)
                </div>
                {blue.rejected_proposals.map((p, i) => (<PatchCard key={`rej-${i}`} patch={p}/>))}
              </>)}

            <div className="kicker pt-1">Safety rail preview</div>
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              {hasRealRail ? (<>
                  <StatusChip label={`${SAFETY_RAIL_HEADING} · real telemetry`} color="green" mono/>
            ...
            {blue.human_review_derivation.length > 0 ? (<div className="space-y-2">
                {blue.human_review_derivation.map((row) => (<div key={row.category} className="rounded-lg border border-amber-200 bg-amber-50/50 p-3">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <StatusChip label={row.category} color="amber"/>
                      <StatusChip label={row.source === "derived_from_retest"
                    ? "derived from retest"
                    : "proposed by agent"} color={row.source === "derived_from_retest" ? "blue" : "neutral"}/>
                    </div>
                    <p className="mt-1.5 text-[11.5px] text-slate-600">{row.reason}</p>
                    {row.derived_from_finding_types.length > 0 && (<p className="mt-1 text-[11px] text-slate-500">
                        finding types:{" "}
                        <span className="font-mono text-slate-700">
                          {row.derived_from_finding_types.join(", ")}
                        </span>
                      </p>)}
                    {row.derived_from_probe_ids.length > 0 && (<p className="mt-0.5 text-[11px] text-slate-500">
                        probes:{" "}
                        <span className="font-mono text-slate-700">
