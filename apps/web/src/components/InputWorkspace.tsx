...
export function InputWorkspace({ value, onChange, onLoadSamples, onResetTab, samplesLoading, loadedFromSample, }: InputWorkspaceProps) {
    ...
    return (<div className="card overflow-hidden">
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-slate-50/70 px-3 py-2.5">
        <div className="flex flex-wrap gap-1">
          {TABS.map((tab) => {
            ...
            return (<button key={tab.key} type="button" onClick={() => setActive(tab.key)} className={clsx("inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-semibold transition", selected
        ...
        <div className="ml-auto flex gap-2">
          <button type="button" onClick={onLoadSamples} disabled={samplesLoading} className="btn-ghost py-1.5 text-xs">
            <Download size={14}/> Load Samples
          </button>
          <button type="button" onClick={() => onResetTab(active)} className="btn-ghost py-1.5 text-xs text-slate-500">
      ...
      <div className="p-4 lg:p-5">
        <textarea key={active} value={current} onChange={(e) => onChange({ ...value, [active]: e.target.value })} spellCheck={false} aria-label={activeTab.label} className={clsx("editor scroll-thin min-h-[360px]", !activeTab.mono && "font-sans")}/>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
          <span className="inline-flex items-center gap-2">
            {loadedFromSample && (<span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-medium">
                loaded from sample
              </span>)}
