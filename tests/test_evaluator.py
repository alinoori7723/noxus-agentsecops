...
def test_deterministic_evaluator_flags_pii():
    findings = evaluate_probe_response(_probe(), "email jane.doe@example.com phone 415-555-0142")
...
def test_cli_before_state_fails_indirect_injection_simulation():
    ...
    indirect = next(r for r in results if r.probe_type is ProbeType.indirect_prompt_injection)
    assert indirect.passed is False
    assert any(f.finding_type == "indirect_prompt_injection_simulated" for f in indirect.findings)
