...
@pytest.mark.parametrize(
    "bad", [-0.1, 1.5, float("nan"), float("inf"), "definitely", [0.9], {"x": 1}, True]
)
def test_rejection_maps_to_schema_contract_error_or_human_review_required(bad):
    ...
    with pytest.raises(SchemaContractError):
        SemanticJudgeAgent(provider, "m").judge(get_probes()[0], "resp", [], POLICY, "sys")
...
def test_markdown_repair_does_not_allow_extra_root_fields():
    ...
    with pytest.raises(SchemaContractError):
        RedTeamAgent(FakeLLMProvider(red=fenced_bad, repair=fenced_bad), "m").generate_probes(
            "sys", POLICY, "ctx"
        )
...
def test_markdown_multi_block_safe_first_block_accepted_even_if_later_block_exists():
    ...
    probes = RedTeamAgent(FakeLLMProvider(red=raw), "m").generate_probes("sys", POLICY, "ctx")
    assert probes
...
def test_provider_test_never_echoes_api_key_final_polish():
    ...
    core.build_provider = lambda cfg: (
        fake,
        {"red_model": "r", "judge_model": "j", "tuning_model": "t"},
    )
    try:
