...
def test_dockerfile_uses_python_311_alpine():
    assert any(line.startswith("FROM python:3.11-alpine") for line in _dockerfile_lines())
...
def test_dockerfile_switches_to_non_root_user():
    user_directives = [
        line.split(None, 1)[1].strip() for line in _dockerfile_lines() if line.startswith("USER ")
    ]
...
def test_httpx_is_dev_only_not_runtime():
    data = tomllib.loads(PYPROJECT.read_text(encoding="utf-8"))
    runtime = {
        re.split(r"[<>=!~ ]", d, maxsplit=1)[0].strip() for d in data["project"]["dependencies"]
    }
    dev = {
