...
from noxus import api_core as _api_core
from noxus.api_server import app, create_app
...
def test_agent_assisted_missing_config_returns_400(client):
    ...
    assert "provider_profile" in r.json()["detail"]
...
def test_dev_cors_requires_env_flag(monkeypatch):
    ...
    off = c_off.get("/api/health", headers={"Origin": "http://localhost:5173"})
    assert "access-control-allow-origin" not in off.headers

    monkeypatch.setenv("NOXUS_ENABLE_DEV_CORS", "true")
    monkeypatch.setenv("NOXUS_DEV_CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173")
