...
from noxus import report as report_mod
...
def test_no_risk_score_label_for_higher_is_better_score():
    ...
    for key in (
        "baseline_readiness_score_label",
        "readiness_gate_score_label",
...
def test_failed_probes_with_zero_readiness_not_labeled_zero_risk():
    rep = _report(
        before=[
            _mk_result(
                "p", ProbeType.pii_leakage, [_mk_finding("pii_leakage", ProbeType.pii_leakage)]
            )
        ],
        after=[
            _mk_result(
                ...
                [_mk_finding("pii_leakage", ProbeType.pii_leakage, severity=Severity.high)],
            )
        ],
        after_score=0,
        state=ReadinessState.HUMAN_REVIEW_REQUIRED,
...
def _routing_report():
    ...
    return _report(
        ...
        after_score=0,
        state=ReadinessState.HUMAN_REVIEW_REQUIRED,
        human_review=["pii", "proprietary_context"],
        meta=meta,
    )
...
def test_human_review_derivation_instance_count_matches_routed_instances():
    model = ui_formatters.build_remediation_model(_routing_report())
    routed = sum(
        len(row["derived_from_finding_instance_ids"]) for row in model["human_review_derivation"]
    )
...
def test_human_review_derivation_type_count_matches_routed_types():
    model = ui_formatters.build_remediation_model(_routing_report())
    routed_types = {
        t for row in model["human_review_derivation"] for t in row["derived_from_finding_types"]
    }
...
def test_human_review_required_with_unresolved_high_risk_findings_has_derivation():
    rep = _report(
        ...
        after=[
            _mk_result(
                ...
                [
                    _mk_finding(
                        "pii_leakage", ProbeType.pii_leakage, severity=Severity.high, probe_id="p1"
                    )
                ],
            )
        ],
    ...
    derived = [
        r for r in model["human_review_derivation"] if r["derived_from_finding_instance_ids"]
    ]
...
def test_secret_patch_primary_source_is_fake_secret_even_when_must_not_appear_exists():
    findings = [
        _mk_finding(
            "fake_secret_exfiltration", ProbeType.fake_secret_exfiltration, probe_id="probe_secret"
        ),
        _mk_finding(
            "must_not_appear_violation",
            ProbeType.proprietary_context_exposure,
...
def test_same_probe_leakage_is_labeled_as_leakage_from_same_probe():
    op = _injection_op(customer_probe="probe_inj")
    groups = remediation.classify_related_findings(op)
    assert any("customer_identifier_leakage" in fid for fid in groups["leakage_from_same_probe"])
    assert not any("customer_identifier_leakage" in fid for fid in groups["same_category_related"])
...
def test_raw_audit_detail_preserved_without_polluting_primary_ui_lineage():
    ...
    assert any("customer_identifier_leakage" in fid for fid in row["secondary_source_finding_ids"])
...
def test_mapping_matrix_values_are_computed_not_hardcoded():
    ...
    assert m["baseline_finding_instance_count"] == sum(len(r.findings) for r in rep.before_results)
    assert m["retest_finding_instance_count"] == sum(len(r.findings) for r in rep.after_results)
    assert m["baseline_failed_probe_count"] == sum(1 for r in rep.before_results if not r.passed)


def _improved_blocked_report(state=ReadinessState.HUMAN_REVIEW_REQUIRED):
    meta = ReportMetadata(
        resolved_finding_count=3, unresolved_finding_count=1, patch_application_count=5
    )
    return _report(
        before=[
            _mk_result(
