...
def _multi_unresolved_report():
    ...
    after = [
        _mk_result(
            ...
            [_mk_finding("pii_leakage", ProbeType.pii_leakage, Severity.high, "p_pii")],
        ),
        _mk_result(
            "p_sec",
        ...
        _mk_result(
            ...
            [
                _mk_finding(
                    "policy_conflict_note", ProbeType.policy_conflict_probe, Severity.low, "p_misc"
                )
            ],
        ),
    ]
    ...
    return _report(
        ...
        after_score=0,
        state=ReadinessState.HUMAN_REVIEW_REQUIRED,
        human_review=["pii", "secrets", "customer_identifier"],
        meta=meta,
    )
...
def test_must_not_appear_never_beats_specific_pii_secret_customer_prompt():
    ...
    for operation, fields, specific, expected in cases:
        op = PatchOperation(
            operation=operation,
            target="policy",
            source_probe_ids=["a_prop", specific.probe_id],
            **fields,
        )
