...
def test_format_detection_label_semantic_llm():
    ...
    assert (
        ui_formatters.format_detection_label(DetectionMode.deterministic) == "[DETERMINISTIC CHECK]"
    )
...
def test_format_readiness_badge_conditional_pass_is_amber():
    ...
    assert badge["headline"] == "CONDITIONAL_PASS — improved, but human review still required"
...
def test_open_risk_model_preserves_proprietary_context_exposure():
    ...
    assert any("proprietary_context_exposure" in r for r in model["proprietary_open_risks"])
    assert "CONDITIONAL_PASS, not fake PASS" in model["proprietary_context_explanation"]
...
def test_evidence_report_model_surfaces_open_risks():
    ...
    assert model["readiness"]["color"] == "amber"
...
def test_demo_timeline_uses_real_report_counts():
    ...
    patching = timeline[3]
    final = timeline[-1]
    assert baseline["evidence_count"] == sum(len(r.findings) for r in report.before_results)
...
def test_red_blue_dashboard_uses_real_safety_rail_preview_from_patched_prompt():
    ...
    preview = ui_formatters.build_red_blue_dashboard_model(report)["blue"]["safety_rail_preview"]
...
def test_red_blue_dashboard_does_not_synthesize_safety_rail_placeholder():
    ...
    deterministic_model = ui_formatters.build_red_blue_dashboard_model(_deterministic_report())
