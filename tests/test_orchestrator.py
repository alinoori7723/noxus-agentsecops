...
def test_deterministic_mode_preserves_baseline_behavior():
    ...
    indirect_before = next(
        r for r in report.before_results if r.probe_type is ProbeType.indirect_prompt_injection
    )
    indirect_after = next(
        r for r in report.after_results if r.probe_type is ProbeType.indirect_prompt_injection
    )
...
def test_agent_assisted_mode_does_not_fake_proprietary_context_pass():
    ...
    report = _agent_assisted(provider)
    proprietary = [
        r for r in report.after_results if r.probe_type is ProbeType.proprietary_context_exposure
    ]
    assert proprietary
