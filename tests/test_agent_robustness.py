...
def test_semantic_judge_normalizes_confidence_when_unambiguous():
    out = normalize_semantic_judgment({"confidence": "High ", "detection_mode": "SEMANTIC_LLM"})
...
def test_red_team_accepts_fenced_json_from_fake_provider():
    ...
    probes = RedTeamAgent(FakeLLMProvider(red=fenced), "m").generate_probes("sys", POLICY, "ctx")
    assert probes
...
def test_policy_tuning_accepts_patch_operations_alias_end_to_end():
    ...
    ps = PolicyTuningAgent(FakeLLMProvider(tuning=alias), "m").propose_patches([], POLICY, "sys")
    ...
    bad = json.dumps(
        {
            "operations": [
                {
                    "operation": "replace_system_prompt",
...
def test_policy_tuning_accepts_allowlisted_policy_path():
    ...
    ps = PolicyTuningAgent(FakeLLMProvider(tuning=good), "m").propose_patches([], POLICY, "sys")
    assert any(op.path == "prompt_injection.detect_indirect_instructions" for op in ps.operations)
...
def test_patch_engine_rejects_unsafe_policy_path_defensively():
    ...
    ps = PatchSet(
        operations=[
            PatchOperation(
                operation=PatchOp.add_control, target="policy", path="../../etc/passwd", value="x"
            )
        ]
    )
    with pytest.raises(SchemaContractError):
...
def test_patch_engine_maps_arbitrary_key_validation_to_schema_contract_error():
    ...
    ps = PatchSet(
        operations=[
            PatchOperation(
                operation=PatchOp.add_control, target="policy", path="bogus_key", value="x"
            )
        ]
    )
    with pytest.raises(SchemaContractError):
...
def test_agent_assisted_red_failure_response_shows_fallback_not_blank():
    ...
    resp = api_core.build_assessment_response(report, mode="agent_assisted", provider_config=cfg)
