...
backend. Release verification: **351 Python tests** (plus **36 frontend tests**).
...
- Release verification: **351 Python tests** (Python core + API; run via
  `pip install -e ".[dev]"` then `pytest -q`), including **35 Milestone 1
  deterministic/regression tests**, plus **36 frontend tests**.
...
.venv/bin/pytest -q                 # full suite: release-verified at 351 Python tests
```
...
pip install -e .
NOXUS_TEST_COUNT=351 uvicorn noxus.api_server:app --reload --port 8787
...
NOXUS_WEB_DIST=apps/web/dist NOXUS_TEST_COUNT=351 \
